...
#################################################################################################################
#
#           This script will help is automating the Perf Stats collection process. This script can be used with
#           nohup to run the process in the background to prevent exiting upon terminal hang or closure. 
# -------------------------------------------------------------------------------------------------------------
#
#          FILE:  collectPerf.sh
#         USAGE:  nohup ./nohupCollectPerf.sh <numberOfIterations> <diskSerialNumber> <timeDuration> &
#       DETAILS:  The arguments provided in the script are explained below. Kindly read to understand further. 
#        AUTHOR:  STALIN STEPIN.
#       CONTACT: stalin.stepin@nutanix.com
#        GITHUB: https://github.com/Sn0d3n/collectPerf
#       COMPANY:  NUTANIX
#       VERSION:  1.0
#       UPDATED:  24-04-2021
...
# numberOfIterations argument implies the number of iteration the collect perf should run. 
numberOfIterations=$1
printf "\nNumber of iterations provided is: %s" "${numberOfIterations}"

# diskSerialNumber argument implies the stargate disk serial number that needs to be entered. 
diskSerialNumber=$2
printf "\nDisk serial number entered is: %s\n" "${diskSerialNumber}"

# timeDuration argument implies how long the collect perf should be running before triggering the "collect_perf stop" command.
timeDuration=$3
printf "Duration (value in minutes) for each iteration entered is: %s\n" "${timeDuration}"
